...
%% 
%% 
Elevator{1,1}.up(1,7) = 1;
%Elevator{1,1}.down(1,9) = 1;
%Elevator{1,1}.state(1,2) = 1;
Elevator{1,1}.floor = 2;
%Elevator{1,1}.inner(1,1) = 1;
...
%             fprintf("又到了")
%          end         
%         if Elevator{1,k}.floor == 9 && Elevator{1,k}.state(1,2) == 1
%             test = 1;
%         end
        %调试结束
    %pause(0.002);
...
%            ***然后让每一个为1 等待的guest的等待时间+4s （从一楼到另一楼的运行时间）
        ...
        if (Elevator{1,k}.state(1,2) && flag1 == 0)%如果上行状态为1 并且第一次出现了上行状态，装载时间
           oritime1 = time; 
           flag1 = 1;
        end
        if (Elevator{1,k}.state(1,2) && flag1 == 1 && (time - oritime1 > 100*4))%如果上行状态为1 并且第一次出现了上行状态，装载时间
           now = now + 1;  
           flag1 = 0; %需要重新装载时间
        ...
        %--------------------------下行状态
        if (Elevator{1,k}.state(1,3) && flag2 == 0)%如果下行状态为1 并且第一次出现了上行状态，装载时间
           oritime2 = time; 
           flag2 = 1;
        end
        if (Elevator{1,k}.state(1,3) && flag2 == 1 && (time - oritime2 > 100*4))%如果上行状态为1 并且第一次出现了上行状态，装载时间
           now = now - 1;  
           flag2 = 0; %需要重新装载时间
           Elevator{1,k}.floor = now;
           %-------------- 时间变化
...
%        ***然后让每一个为1 等待的guest的等待时间+5 （停层的时间）
       ...
       if Elevator{1,k}.StopFloor(1,now_floor) == 1 %到了该停止的楼层
          ...
          SSwitch_dir();%如果到了极限层，要变成静止状态
          SGet_Limit();
          SGet_StopFloor();%如果是静止状态，得到几个stopfloor，先让电梯进入状态动起来
          SGet_dir();
          %**************
